...
export async function parseNewsRSS(xml){
 ...
 for(const block of blocks){
  ...
  if(!title||!sourceUrl||!keywords.test(`${title} ${summary} ${category}`))continue;
...
export async function refreshNews(env,{notify=true}={}){
 ...
 if(notify&&fresh.length)await sendNewsPush(env,fresh.slice(0,3));
 return {saved:rows.length,newCount:fresh.length};
}
