...
export function rentGrowthProjection(price,monthlyRent,annualIncrease,years){
 if(!validNumbers([price,monthlyRent,annualIncrease,years])||price<=0||monthlyRent<0||annualIncrease<=-100||annualIncrease>500||!Number.isInteger(years)||years<1||years>40)throw Error('Fiyat, kira, artış oranı ve 1–40 yıl arasındaki süreyi kontrol edin.');
 const rows=[];let cumulative=0,paybackMonth=null;
 for(let year=1;year<=years;year++){
  const rent=monthlyRent*Math.pow(1+annualIncrease/100,year-1),annualRent=rent*12;
  for(let month=1;month<=12;month++){cumulative+=rent;if(paybackMonth===null&&cumulative>=price)paybackMonth=(year-1)*12+month}
  rows.push({year,monthlyRent:rent,annualRent,cumulative});
 }
 return {rows,totalRent:cumulative,paybackMonth,paybackYears:paybackMonth===null?null:paybackMonth/12};
}
